...
Expression using Cohen's *d* To check the genes that are available 
...
```{r}
table(pred_res$pearson_WKNN_DM$predRes, wang_cellTypes)
```

Prediction results for spearman as the similarity metric:

```{r}
table(pred_res$spearman_WKNN_DM$predRes, wang_cellTypes)
```
